					...
					<div class="col-md-12">
						<div class="table-responsive">
							<table class="table table-striped custom-table mb-0 datatable">
								...
								<tbody>
									@if(!empty($get_all_reimbursement_req))
									@foreach($get_all_reimbursement_req as $reimbursement_req)
									<tr>
										<td>{{strtoupper($reimbursement_req->travel_req_travelid)}}</td>
										<td>{{$reimbursement_req->travel_amt}} INR</td>
										<td>
										@if($reimbursement_req->travel_doc != null)
											@php 
												$travelDoc = json_decode($reimbursement_req->travel_doc, true);
											@endphp
											@foreach($travelDoc as $td)
												@php
													$extension = pathinfo($td, PATHINFO_EXTENSION);
													$imageExtensions = ['jpg', 'jpeg', 'png', 'gif'];
												@endphp
												@if(in_array($extension, $imageExtensions))
													<a href="{{ asset('public/uploads/traveldocument/' . $td) }}" class="image-popup">
														<img style="width:50px; height:50px" src="{{ asset('public/uploads/traveldocument/' . $td) }}" alt="">
													</a></br>
												@else
													<a href="{{ asset('public/uploads/traveldocument/' . $td) }}" target="_blank">{{ $td }}</a></br>
												@endif
											@endforeach
										@else
											NULL
										...
										<td>{{$reimbursement_req->hotel_amt}} INR</td>
										<td>
										@if($reimbursement_req->hotel_doc != null)
											@php 
											...
											@foreach($travelDoc as $td)
												...
												@endphp
												@if(in_array($extension, $imageExtensions))
													<a href="{{ asset('public/uploads/Hoteldocument/' . $td) }}" class="image-popup">
														<img style="width:50px; height:50px" src="{{ asset('public/uploads/Hoteldocument/' . $td) }}" alt="">
													</a></br>
												@else
													<a href="{{ asset('public/uploads/Hoteldocument/' . $td) }}" target="_blank">{{ $td }}</a></br>
												@endif
											@endforeach
										@else
											NULL
										...
										<td>{{$reimbursement_req->loc_amt}} INR </td>
										<td>
										@if($reimbursement_req->loc_doc != null)
											@php 
											...
											@foreach($travelDoc as $td)
												...
												@endphp
												@if(in_array($extension, $imageExtensions))
													<a href="{{ asset('public/uploads/localtraveldocument/' . $td) }}" class="image-popup">
														<img style="width:50px; height:50px" src="{{ asset('public/uploads/localtraveldocument/' . $td) }}" alt="">
													</a></br>
												@else
													<a href="{{ asset('public/uploads/localtraveldocument/' . $td) }}" target="_blank">{{ $td }}</a></br>
												@endif
											@endforeach
										@else
											NULL
										...
										<div class="col-md-12">
											<div class="input-block mb-3">
												...
												<select class="form-select form-control"  name="travelid" id="travelid">
													...
													@foreach($get_all_travel_req as $trvreq)
													<option value="{{$trvreq->id}}" dataid="{{$trvreq->userid}}">{{strtoupper($trvreq->travelid)}}</option>
													@endforeach
												</select>
