                ...
                <div class="content container-fluid">
					...
					<form action="{{ url('admin/attendance') }}" method="GET">
						<div class="row filter-row">
							...
							<div class="col-sm-6 col-md-2"> 
								<div class="input-block mb-3 form-focus select-focus">
									<select class="select floating" name="year" id="year"> 
										@for ($y = date('Y'); $y >= date('Y') - 10; $y--)
											<option value="{{ $y }}" {{ $y == $year ? '' : '' }}>{{ $y }}</option>
										@endfor
									</select>
							...
							<div class="col-sm-6 col-md-3">  
								<div class="d-grid">
									<a href="{{ url('admin/attendance') }}" class="btn btn-info"> Reset </a>  
								</div>
							</div>     
						</div>
					</form>
                        ...
                        <div class="col-lg-12">
							...
							<thead>
								<tr>
									...
									@endphp
									@foreach (range(1, $numDaysInMonth) as $day)
										<th>{{ $day }} {{$monthName }}</th>
									@endforeach
								</tr>
							</thead>
							<tbody>
								...
								@foreach ($userAttendance as $userId => $userDays)
								@php
								...
								<tr>
									<td>
										<h2 class="table-avatar">
											<a class="avatar avatar-xs" href="{{ url('admin/employees/profile') }}/{{ strtolower($userDays[key($userDays)]['emp_id']) }}">
											@if($userDays[key($userDays)]['image'] != null)
												<img src="{{ url('public/uploads/profile') }}/{{ $userDays[key($userDays)]['image'] }}" alt="User Image">
											@else
												<span class="em-name-show">{{$initials}}</span>
											@endif
											</a>
											<a href="{{ url('admin/employees/profile') }}/{{ strtolower($userDays[key($userDays)]['emp_id']) }}">{{ $userDays[key($userDays)]['firstname'] }} {{ $userDays[key($userDays)]['lastname'] }}</a>
										</h2>
									</td>
									@foreach (range(1, $numDaysInMonth) as $day)
