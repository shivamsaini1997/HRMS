						...
						<div class="col-md-12">
							<div class="table-responsive">
								<table class="table table-striped custom-table datatable">
									...
									<tbody>
									@if(!empty($get_all_consultant_salary))
									@foreach($get_all_consultant_salary as $const)
										<tr>
											<td>
												<h2 class="table-avatar">
													<a href="profile.html" class="avatar"><img src="{{url('public/uploads/profile')}}/{{$const->image}}" alt="User Image"></a>
													<a href="javascript:void(0)">{{$const->firstname}} {{$const->lastname}}</a>
												</h2>
											</td>
											<td>{{$const->sal_month}}</td>

											<td>{{$const->payable_days}}</td>
											<td>{{$const->monthly_sal}}</td>
											<td>{{$const->consolidated_fee}}</td>
											<td>{{$const->tds}}</td>
											<td>{{$const->net_pay}}</td>
											<td>{{$const->type}}</td>
											<td>
												@if(!empty($const->project_details))
													@foreach(json_decode($const->project_details) as $project)
														<p>Project Number: {{$project->projectnumber}}</p>
														<p>Project Cost: {{$project->projectcost}}</p>
														<p>Number of Days: {{$project->numberofday}}</p>
														<hr> 
													@endforeach
												@else
													Null
											...
											<td><a class="btn btn-sm btn-primary" href="{{url('/admin/consultant-salary')}}/{{strtolower($const->emp_id)}}/{{$const->id}}">Generate Slip</a></td>
											<td class="text-end">
												<div class="dropdown dropdown-action">
													...
													<div class="dropdown-menu dropdown-menu-right">
														...
														<a class="dropdown-item dl-cs" href="#" data-bs-toggle="modal" data-bs-target="#delete_salary" data-id="{{$const->id}}" data-us-id="{{$const->userid}}"><i class="fa-regular fa-trash-can m-r-5"></i> Delete</a>
													</div>
												</div>
											</td>
										</tr>
										...
										<div class="col-sm-6"> 
											<div class="input-block mb-3">
												...
												<option value=""> -- Select Consultant -- </option>
													@foreach($all_emp_sal as $employee)
													<option value="{{$employee->userid}}" dataid="{{$employee->email}}" data-salary="{{$employee->salary_amt}}">{{$employee->firstname}} {{$employee->lastname}}</option>
													@endforeach
