...
function travelclone() {
    const removerclass = document.getElementById("removerow");
    removerclass.classList.remove('d-none');

    const clonedRow = $('#travelrow').clone();
    const newRowId = 'travelrow_' + sharerCount;

    clonedRow.attr('id', newRowId);

    // Update ids and names for all input elements inside the cloned row
    clonedRow.find("input, select").each(function () {
        const currentId = $(this).attr('id');
        const currentName = $(this).attr('name');

        // Update id and name attributes by appending the sharerCount
        const newId = currentId ? currentId.replace(/\d+/, sharerCount) : null;
        const newName = currentName ? currentName.replace(/\d+/, sharerCount) : null;
    ...
    const removeButton = $('<a href="javascript:void()" onclick="removeTravelRow(this)"><ul class="icons-list justify-content-end mt-3"><li i class="plusrowtravel"><i class="fe fe-minus"></i></li></ul></a>');
    clonedRow.find(".removerow").empty().append(removeButton);

    // Append the cloned and modified row after the last row
    clonedRow.insertAfter('.travelrow:last');

    sharerCount += 1;
}

// Function to remove the cloned row
function removeTravelRow(button) {
    // Find the parent row and remove it
    $(button).closest('.travelrow').remove();
}
...
function hotelclone() {
    const removerclass = document.getElementById("removerow1");
    removerclass.classList.remove('d-none');

    const clonedRow = $('#hotelrow').clone();
    const newRowId = 'hotelrow_' + hotelCount;
    ...
    clonedRow.find("input, select").each(function () {
        ...
        // Update id and name attributes by appending the hotelCount
        const newId = currentId ? currentId.replace(/\d+/, hotelCount) : null;
        const newName = currentName ? currentName.replace(/\d+/, hotelCount) : null;
    ...
    // Add a remove button to the cloned row
    const removeButton = $('<a href="javascript:void()" onclick="removeHotelRow(this)"><ul class="icons-list justify-content-end mt-3"><li i class="plusrowtravel"><i class="fe fe-minus"></i></li></ul></a>');
    clonedRow.find(".removerow1").empty().append(removeButton);

    // Append the cloned and modified row after the last row
    clonedRow.insertAfter('.hotelrow:last');
...
function removeHotelRow(button) {
    // Find the parent row and remove it
    $(button).closest('.hotelrow').remove();
}
...
function submitForm() {
	...
	// Collect data from dynamically added travel rows
	$(".travelrow").each(function(index) {
		var travelData = $(this).find(":input").serializeArray();
		formData = formData.concat(travelData.map(function(item) {
			return { name: "travel_" + index + "_" + item.name, value: item.value };
		}));
	});

	// Collect data from dynamically added hotel rows
	$(".hotelrow").each(function(index) {
		var hotelData = $(this).find(":input").serializeArray();
		formData = formData.concat(hotelData.map(function(item) {
			return { name: "hotel_" + index + "_" + item.name, value: item.value };
		}));
	});
	...
	$.ajax({
		type: "POST",
		url: "travel-request",
		data: formData.reduce(function(obj, item) {
			obj[item.name] = item.value;
			return obj;
		}, {}),
		success: function(response) {
			triggerAlert('Travel request added successfully', 'success');
...
function validateForm() {
	$(".travelrow :input[required]").removeClass('is-invalid');

	var isValid = true;

	$(".travelrow :input[required]").each(function() {
		var field = $(this);

		if (field.val().trim() === '') {
			isValid = false;
			field.addClass('is-invalid');
			triggerAlert('Please enter ' + field.attr('placeholder') + '.', 'error');
			field.focus();
			return false;
		}
	});
...
function triggerAlert(message, type) {
	// Your alert triggering logic goes here
...
// Function to clone travel row
// function travelclone() {
//     var clonedRow = $("#travelrow").clone();
//     clonedRow.find(":input").val('');
//     $("#travelrow").after(clonedRow);
// }

// // Function to clone hotel row
// function hotelclone() {
//     var clonedRow = $("#hotelrow").clone();
//     clonedRow.find(":input").val('');
//     $("#hotelrow").after(clonedRow);
// }
